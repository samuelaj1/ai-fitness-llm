                ...
                <div class="dropdown-menu row" aria-labelledby="topnav-dashboard" v-if="hasItems(item)">
                  <template v-for="(subitem, index) of item.subItems">
                    <router-link :key="index" class="col dropdown-item side-nav-link-ref" v-if="!hasItems(subitem)"
                                 :to="subitem.link">{{ $t(subitem.label) }}
                    </router-link>
                    <div class="dropdown" v-if="hasItems(subitem)" :key="index">
                      ...
                      <div class="dropdown-menu">
                        <router-link v-for="(subSubitem, index) of subitem.subItems" :key="index" :to="subSubitem.link"
                                     class="dropdown-item side-nav-link-ref">{{ $t(subSubitem.label) }}
                        </router-link>
                      </div>
                    </div>
                  </template>
...
<script>
import {menuItems} from "./vendor-menu";
/**
...
export default {
  ...
  computed: {
    filteredMenuItems() {
      const loggedUser = this.$store.getters.GET_USER_INFO; // Get logged-in user info
      const userRole = loggedUser.roles?.[0] || ''; // Default to an empty string if no role is found
      const userPermissions = loggedUser.permissions || []; // Default to an empty array if no permissions are found

      // Filter menu items based on whether the user has permission for the item
      return this.menuItems.filter(item => {
        // If the user's role is 'vendor_manager' and the item requires specific permissions
        ...
        // If the item doesn't require permissions or the role isn't 'vendor_manager', show the item
        return item;
      });
    }
  },
